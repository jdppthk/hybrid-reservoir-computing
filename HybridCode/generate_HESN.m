function ESN = generate_HESN(NetArgs)
...
% Do the same thing for those nodes connected to the model output
for k=1:num_leftover_nodes
    W_in(leftover_connections(k),leftover_inputs(k)) = NetArgs.input_scale*(2*rand()-1);
end

moutput_conn_per_input = floor((NetArgs.N-RNN_input_size)/NetArgs.M);
for k=NetArgs.M+1:2*NetArgs.M
    W_in(connections((k-(NetArgs.M+1))*moutput_conn_per_input+1+RNN_input_size:...
        (k-NetArgs.M)*moutput_conn_per_input+RNN_input_size),k) = ...
        NetArgs.moutput_scale*(2*rand(moutput_conn_per_input,1)-1);
end

num_leftover_nodes = (NetArgs.N-RNN_input_size)-moutput_conn_per_input*NetArgs.M;
leftover_connections = connections((end-num_leftover_nodes+1):end);
leftover_inputs = randperm(NetArgs.M, num_leftover_nodes)+NetArgs.M;

for k=1:num_leftover_nodes
    W_in(leftover_connections(k),leftover_inputs(k)) = NetArgs.moutput_scale*(2*rand()-1);
end
